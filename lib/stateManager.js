module.exports = {
	...
	set: {
		...
		ActiveIdentifier: async function(identifier, callback) {
			const matchedInput = this.inputs.find(input => input.identifier === identifier)
			if (!matchedInput) {
				this.log.easyDebug(`${this.name} - Requested input identifier ${identifier} not found, ignoring`)
				callback()
				return
			}
			const source = matchedInput.key
